...
async function handleRequestCode(request, env, corsHeaders) {
  ...
  // For now, log it (you can see in Cloudflare dashboard)
...
async function handleCreateCheckout(request, env, corsHeaders) {
  ...
  // Create Creem checkout (Test Mode uses test-api.creem.io)
  const response = await fetch('https://test-api.creem.io/v1/checkouts', {
    ...
    body: JSON.stringify({
      product_id: env.CREEM_PRODUCT_ID || 'prod_52YqViVwDNmYS5BOhcxVfV',
...
async function handleCreemWebhook(request, env, corsHeaders) {
  ...
  if (event === 'subscription.active' || event === 'subscription.created') {
